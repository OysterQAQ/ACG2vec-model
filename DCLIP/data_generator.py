...
class DCLIPIterableDataset(torch.utils.data.IterableDataset):
    def __init__(self, danbooru_start=0, danbooru_end=2996459, pixiv_start=0, pixiv_end=108489283, offset=1000, preprocess=_transform(224)):
        super(DanbooruIterableDataset).__init__()
        assert danbooru_end > danbooru_start, "this example code only works with end >= start"
        self.danbooru_start = danbooru_start
        self.danbooru_end = danbooru_end
        self.pixiv_start = pixiv_start
        self.pixiv_end = pixiv_end
        self.danbooru_index = danbooru_start
        self.pixiv_index = pixiv_start
        self.offset = offset
        self.redis_index_key = 'DCLIP_index'
        self.redis_conn = redis.Redis(host='local.ipv4.host', port=6379, password='', db=0)
        self.sql_danbooru = 'select id,path,tags from danbooru_illust where id > %s limit %s'
    ...
    def _sample_generator(self, worker_id, start, end):
        ...
        if worker_id % 2 == 0:
            ...
            while index < end:
                ...
                try:
                    data_from_db = pd.read_sql(self.sql_danbooru, self.engine, params=[index, self.offset])
                except:
                    time.sleep(10)
                    continue
            ...
            while index < end:
                # time_start = time.time()
                try:
                    data_from_db = pd.read_sql(self.sql_pixiv, self.engine, params=[index, self.offset])
                except:
                    time.sleep(10)
                    continue
                # data_from_db = pd.read_sql(self.sql, self.engine, params=[index,self.offset])
    ...
    def __len__(self):
        return (self.danbooru_end - self.danbooru_start)+(self.pixiv_end - self.pixiv_start)
    ...
    def __iter__(self):
        ...
        if worker_info is None:  # single-process data loading, return the full iterator
            ...
            ##如果被2整除就从pixiv取 反之从danbooru取 两者的per_worker不一样
            if worker_id%2==0:
                per_worker = int(math.ceil((self.danbooru_end - self.danbooru_start) / float(worker_info.num_workers/2)))
                iter_start = self.danbooru_start + worker_id/2 * per_worker
                iter_end = min(iter_start + per_worker, self.danbooru_end)
                print("danbooru,begin:"+str(iter_start)+",end:"+str(iter_end))
            else:
                per_worker = int(
                    math.ceil((self.pixiv_end - self.pixiv_start) / float(worker_info.num_workers / 2)))
                iter_start = self.pixiv_start + worker_id / 2 * per_worker
                iter_end = min(iter_start + per_worker, self.pixiv_end)
                print("pixiv,begin:" + str(iter_start) + ",end:" + str(iter_end))
...
#     ds = DCLIPIterableDataset(danbooru_start=5000, danbooru_end=6000, pixiv_start=5000, pixiv_end=6000, offset=10, )
#     dataloader = torch.utils.data.DataLoader(ds, num_workers=10)
