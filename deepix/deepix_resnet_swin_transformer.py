...
class deepix(Model):
    def __init__(self, path, name=None, **kwargs):
        ...
        self.deepdanbooru = load_deepdanbooru_pretrained_model(path)
        self.layer = BasicLayer(dim=int(1024),
                                input_resolution=(16,
                                                  16,),
                                depth=4,
                                num_heads=32,
        ...
        self.view_predict = self._output_layer('view_predict', 10, 'softmax')
        self.sanity_predict = self._output_layer('sanity_predict', 10, 'softmax')
        self.restrict_predict = self._output_layer('restrict_predict', 3, 'softmax')
        self.x_restrict_predict = self._output_layer('x_restrict_predict', 3, 'softmax')

    def _output_layer(self, output_name, output_dim, output_activation):
        return layers.Dense(output_dim, activation=output_activation, name=output_name, dtype="float32")

    def call(self, x, training=None, **kwargs):
        x = self.deepdanbooru(x)
        print(x.shape)
        # B, H, W, C = x.shape
        B, H, W, C = x.get_shape().as_list()
        x = tf.reshape(x, shape=[-1, 16 * 16, 1024])

        x = self.layer(x)
        print(x)
        x = self.norm(x)
        x = tf.reduce_mean(x, axis=1)
        ouput = [self.bookmark_predict(x), self.view_predict(x), self.sanity_predict(x), self.restrict_predict(x),
                 self.x_restrict_predict(x)]
        # print(x.shape)
        return ouput
...
def create_swin_deepix(path):
    ...
    x = tf.keras.layers.Lambda(lambda x: tf.reduce_mean(x, axis=1), name='deepix_reduce_mean_layer')(x)
...
class BasicLayer(tf.keras.layers.Layer):
    ...
    #
    # def get_config(self):
    #     base_config = super( self).get_config()
    #     return base_config
    #
...
# model.save('/Volumes/Data/oysterqaq/Desktop/1')
# # model=SwinTransformer(model_name='swin_base_224',pretrained=False)
# # model.compile(
# #         optimizer='adam',
...
#                   }
# model.compile(loss=multi_loss,optimizer='adam')
#
...
#     print(l.name)
# model.save('/Volumes/Data/oysterqaq/Desktop/2')

# model = tf.keras.models.load_model('/Volumes/Data/oysterqaq/Desktop/2')
# model.summary()
# input_shape = (4, 512, 512, 3)
# model.build([1, 512, 512, 3])
# x = tf.random.normal(input_shape)
# print(model(x))
# model=tf.keras.models.load_model('/Volumes/Data/oysterqaq/Downloads/deepdanbooru-v4-20200814-sgd-e30/model-resnet_custom_v4.h5')
# ouput_model_arch_to_image(model,'deepdanbooru-v4.jpg')
# model.summary()
